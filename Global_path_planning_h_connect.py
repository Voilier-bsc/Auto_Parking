...
STEER_CHANGE_COST = 0  # steer angle change penalty cost
STEER_COST = 0  # steer angle change penalty cost
    ...
    while True:
        ...
        for neighbor in get_neighbors(current_node_A, obstacle_list, obstacle_kd_tree, "A"):
            ...
            if neighbor not in OpenList_A or OpenList_A[neighbor_ind].cost > neighbor.cost:
                # heapq.heappush(priority_q_A, (calc_cost(neighbor, target_node), neighbor_ind))
...
def calc_next_node(current_node, d, steer, obstacle_list, kd_tree, AB):
    ...
    if not Vehicle.check_vehicle_collision(x,y,yaw,obstacle_list,kd_tree,AB):
